...
from loaddata import preprocess_data

parser = argparse.ArgumentParser()
parser.add_argument("--model", default='./ernie-3.0-base-zh', type=str, help="backbone of encoder.")
parser.add_argument("--train_path", default="data/reward_datasets/sentiment_analysis/train.tsv", type=str, help="The path of train set.")
parser.add_argument("--dev_path", default="data/reward_datasets/sentiment_analysis/dev.tsv", type=str, help="The path of dev set.")
parser.add_argument("--save_dir", default="checkpoints/reward_model/sentiment_analysis", type=str, required=False, help="The output directory where the model predictions and checkpoints will be written.")
...
parser.add_argument("--img_log_dir", default="logs/reward_model/sentiment_analysis", type=str, help="Logging image path.")
parser.add_argument("--img_log_name", default="ERNIE Reward Model", type=str, help="Logging image file name.")
...
def train():
    ...
    train_example = preprocess_data(args.train_path, tokenizer, args.max_seq_len,args.batch_size)
    dev_example = preprocess_data(args.dev_path, tokenizer, args.max_seq_len,args.batch_size)
    train_dataloader = DataLoader(train_example, shuffle=False, collate_fn=default_data_collator,
                                  batch_size=args.batch_size)
    #pdb.set_trace()
    eval_dataloader = DataLoader(dev_example, shuffle=False, collate_fn=default_data_collator,
                                 batch_size=args.batch_size)
